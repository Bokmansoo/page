...
export function UnifiedProductIntakePanel() {
  ...
  async function start(event: FormEvent) {
    ...
    try {
      ...
      setNotice("입력 자료를 고정했습니다. 현재 LangGraph 실행 상태를 표시합니다.");
  ...
  return <main className="mx-auto max-w-4xl space-y-6 px-4 py-8 sm:px-6" aria-labelledby="intake-title">
    <header className="space-y-2"><p className="text-sm font-semibold text-emerald-600">LG-12I 상품 입력</p><h1 id="intake-title" className="text-2xl font-bold text-slate-900">상품 입력 방식</h1><p className="text-sm text-slate-600">선택한 방식은 동일한 LangGraph 입력 경로에서 근거·확인·콘텐츠 기준으로 이어집니다.</p></header>
    ...
    <form onSubmit={start} className="space-y-6">
      ...
      <button disabled={busy} className="rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-emerald-300">{busy ? "처리 중…" : "상품 입력 시작"}</button>
    </form>
    {run && <section className="space-y-3 rounded-2xl border border-indigo-200 bg-indigo-50 p-5" aria-label="LangGraph 진행 상태"><p className="text-xs font-semibold text-indigo-700">PRODUCTION LANGGRAPH</p><h2 className="text-lg font-bold text-slate-900">{stageText}</h2><p className="text-sm text-slate-700">선택 방식: {mode} · {generationMode} · {selectedChannels(channels).join(", ")}</p>{isRecovery(run.current_stage) && <div className="rounded-lg bg-white p-3 text-sm text-slate-700"><p>현재 입력 자료를 완료하지 못했습니다. 같은 실행을 재시도하거나 위에서 사진·직접 입력 방식으로 바꿔 새 immutable 입력을 시작할 수 있습니다.</p><button type="button" disabled={busy} onClick={retry} className="mt-3 rounded-lg border border-indigo-300 px-3 py-2 text-sm font-semibold text-indigo-700">같은 실행 다시 시도</button></div>}{run.current_stage === "master_ready" && <a className="inline-flex rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold text-white" href={"/workspace/projects/" + projectId + "/planning?runId=" + run.run_id}>기존 상세페이지 흐름으로 계속</a>}</section>}
