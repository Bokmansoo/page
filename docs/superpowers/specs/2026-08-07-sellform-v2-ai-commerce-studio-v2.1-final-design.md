...
이 문서는 Sellform을 단순한 “LangGraph 데모”가 아니라, 판매자가 상품 사진과 최소 정보만 입력해 판매 가능한 상세페이지를 만들고 대화로 부분 수정할 수 있는 AI Commerce Studio로 완성하기 위한 단일 제품·기술 기준이다.
...
> 판매자가 상품 사진과 확인 가능한 정보를 넣으면, AI가 카테고리·상품·판매 채널에 맞는 전략과 장면을 설계하고, 제품 정체성을 보존한 이미지와 정확한 한국어 카피를 조립해 수정 가능한 상세페이지와 채널별 출력물을 만든다.

최종 제품은 다음 경험을 제공해야 한다.

1. 사진 1~3장과 상품 정보만으로 초안을 시작할 수 있다.
2. 링크 수집에 실패해도 직접 업로드 사진과 판매자 입력으로 계속할 수 있다.
...
| 모드 | 동작 |
| --- | --- |
| `quick` | 비용 승인 뒤 정상 품질 gate를 자동 통과하며 기획·카피·이미지·조립·QA까지 진행한다. 치명적인 사실 충돌, 비용 한도 초과, 권리·정체성·정책 위험은 자동 승인하지 않고 멈춘다. |
| `expert` | 입력, 사실, 기획, 비용, 이미지와 최종 결과를 단계별로 확인한다. |

FAST-01. 시작 화면에서 `빠른 생성`과 `단계별 검토`를 선택하며 빠른 생성을 추천값으로 표시한다.  
FAST-02. 선택한 모드는 run snapshot과 checkpoint에 저장하고 새로고침·재개 후 유지한다.  
FAST-03. 빠른 생성은 승인 gate를 삭제하지 않고 정책에 따라 안전한 응답을 자동 제출한다.  
FAST-04. 사실 충돌, 승인되지 않은 claim, 비용 한도 초과, 권리·정체성·정책 위험은 모드와 관계없이 사용자에게 확인한다.  
FAST-05. 단계별 검토는 각 interrupt의 근거·비용·영향 범위를 보여준다.  
...
| Domain Data | 사실, Prompt Pack, artifact version, job, page version, export history |
...
Golden Dataset은 최초 5개 카테고리마다 최소 3개 제품을 포함한다. 각 제품에는 입력 자산, 승인 사실, 기대 카테고리, 필수 섹션, 금지 claim, 정체성 기준, 기대 출력 manifest와 사람이 평가한 기준 결과가 포함된다.
...
```text
FactSnapshot
  → CategoryPromptPackVersion + ChannelPackVersion
...
다음은 정상 provider 상태와 사용자 승인 대기 시간을 제외한 빠른 생성 모드의 제품 목표다. provider 장애 시 거짓 완료 시간을 약속하지 않고 지연 원인과 갱신된 예상 시간을 표시한다.

SLO-01. 상품 분석·기획 초안의 목표 시간은 1분 이내다.  
SLO-02. 카피·스토리보드·레이아웃 초안의 목표 시간은 3분 이내다.  
SLO-03. AI 이미지가 포함된 일반 상세페이지의 목표 시간은 10~15분이다.  
...
18. 각 Sprint 코드리뷰 문서에 코드 위치, 자동 테스트, 수동 검증, 잔여 위험이 기록된다.
