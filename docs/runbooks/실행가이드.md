...
PowerShell 창을 세 개 열어 아래 순서대로 실행한다. `npm.cmd`를 사용하면 PowerShell의
스크립트 실행 정책 때문에 `npm.ps1`이 차단되는 문제를 피할 수 있다.

#### 첫 번째 PowerShell 창: PostgreSQL
...
PowerShell이 스크립트 실행을 차단하면, 같은 창에서 아래 명령으로 실행한다.
...
백엔드는 `http://127.0.0.1:8001`에서 실행된다.

#### 세 번째 PowerShell 창: 프론트엔드
...
브라우저에서 `http://localhost:3000/workspace`를 연다.

### 3. CMD로 실행

CMD를 선호하면 아래 기존 명령을 사용한다.
...
### JPG/PNG 다운로드용 Chromium 최초 설치

Sellform은 상세페이지 미리보기와 동일한 화면을 JPG/PNG로 캡처하기 위해 Playwright
Chromium을 사용한다. Python 패키지 설치와 브라우저 설치는 별도이므로, 백엔드를 처음
설치했거나 Playwright가 업데이트된 뒤에는 아래 명령을 한 번 실행한다.
...
```powershell
cd C:\page\backend
uv run python -c "from playwright.sync_api import sync_playwright; p=sync_playwright().start(); print(p.chromium.executable_path); p.stop()"
```

출력된 경로에 `chrome-headless-shell.exe` 또는 Chromium 실행 파일이 있으면 준비된 상태다.
브라우저가 없으면 Sellform은 원본 Playwright 예외 전체를 화면에 노출하지 않고 위 설치
명령을 안내하며, 해당 export 작업을 실패 상태로 기록한다.

### Chrome에서 저장 위치를 매번 선택하기

JPG/PNG는 Chrome의 일반 다운로드 기능으로 저장된다. 저장할 폴더를 매번 선택하고 우측 상단
다운로드 목록에도 기록하려면 Chrome 주소창에서 아래 주소를 연다.

```text
chrome://settings/downloads
```

`다운로드 전에 각 파일의 저장 위치 확인`을 켠다. 이 설정은 브라우저 개인 설정이므로
Sellform 화면에서 자동으로 변경할 수 없다.
...
Playwright Python 패키지만 설치되고 캡처용 Chromium이 없을 때 발생한다.
...
설치 후 백엔드를 다시 시작하고 JPG/PNG 다운로드를 재시도한다.
...
DB가 실행된 뒤 PowerShell에서는 `.\run_backend.ps1`, CMD에서는 `run_backend.cmd`를 다시 실행한다.

### 프론트에서 `Failed to fetch`

1. `http://127.0.0.1:8001/`이 열리는지 확인한다.
2. 백엔드를 실행한 PowerShell 또는 CMD 창의 오류를 확인한다.
3. `8001` 포트 충돌 여부를 확인한다.
...
- FastAPI 문서 `http://127.0.0.1:8001/docs`: HTTP `200`
